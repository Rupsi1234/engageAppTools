...
module.exports = {
	...
	productbtn: selectorFile.css.ComproEngage.indexMenu.productbtn,
	...
	getData_productValue: async function (productbtnName) {
		await logger.logInto(await stackTrace.get());
		var obj = [];
		await action.waitForDisplayed(this.productbtn);
		var list = await action.findElements(this.productbtn);
		if (productbtnName) {
			for (var i = 0; i < list.length; i++) {
				if ((await action.getText(this.productbtn + i)) == productbtnName) {
					obj[0] = {
						productbtn: ((await action.getElementCount(this.productbtn + i + "] p")) > 0) ? await action.getText(this.productbtn + i + "] p") : null,
						currentlbl: ((await action.getElementCount(this.productbtn + i + "] " + this.currentlbl)) > 0) ? await action.getText(this.productbtn + i + "] " + this.currentlbl) : null,
					}
					break;
				}
			}
			...
			for (var i = 0; i < list.length; i++) {
				console.log(this.productbtn + i + "]" + this.currentlbl);
				obj[i] = {
